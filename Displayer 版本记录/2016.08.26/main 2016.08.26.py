...
2016.08.26 完成字体高级编辑功能[字体设置、字体透明度、字体位置]
2016.08.27 *（可选） 完成图片编辑背景色填充设置
'''
import sys  
from PyQt5 import *
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, parent = None):
        ...
        self.font_R9=0
        #关于图片
        self.DisplayPicture=QPixmap("AXLogo.png")#读取图片 AXLogo.png
        self.MDP_Zoom_R=1
        self.MDP_Rote_R=0
        self.MDP_Opacity_R=0.3
        self.MDP_Move_W=0
        self.MDP_Move_H=0
        self.initUI()
    # 控制面板部分
    ...
    def initUI(self):              
        ...
        self.setWindowTitle("安夏电子席卡   ——V1.2") 
...
class FontViewer(QDialog):
    ...
    def Group_Control(self, grpname):
        ...
        row=row+1
        self.Fort_Lable=QLabel(" ")
        FE_gridLayout0_0.addWidget(self.Fort_Lable, row,0)
    ...
    def LayoutSetUp(self): 
        #设置主要控件
        self.mainVertLayout = QVBoxLayout()
        self.mainVertLayout.addWidget(self.Group_Picture_Show("字体预览"))
    ...
    def FE_Opacity(self,FE_Opacity_Rate):
        #设置透明
        self.FE_Opacity_R=FE_Opacity_Rate
        self.FE_QGPixmap.setOpacity(float(FE_Opacity_Rate))
    ...
    def FE_Move_W_FX(self,FE_Move_W_Value):
        self.FE_Move_W=FE_Move_W_Value
        #设置位置
        self.FE_QGPixmap.setPos(self.FE_Screen.width()/2+self.FE_Move_W,self.FE_Screen.height()/2+self.FE_Move_H)
    ...
    def FE_Move_H_FX(self,FE_Move_H_Value):
        self.FE_Move_H=FE_Move_H_Value
        #设置位置
        self.FE_QGPixmap.setPos(self.FE_Screen.width()/2+self.FE_Move_W,self.FE_Screen.height()/2+self.FE_Move_H)
