...
class MainWindow(QMainWindow):
    ...
    def Group1(self, grpname):
        ...
        Name_Input_button = QPushButton("名字输入设置")
        ...
        gridLayout.addWidget(self.Name_Input_Lable, row, 2)
        row = row + 1
        
        gridLayout.addWidget(QLabel("显示设置:"), row, 0)
        Fort_Set_Button = QPushButton("字体详细设置")
        gridLayout.addWidget(Fort_Set_Button, row, 1)
        Fort_Set_Button.clicked.connect(self.OnsetFont)
        self.Fort_Lable=QLabel(" ")
        gridLayout.addWidget(self.Fort_Lable, row, 2)
        row = row + 1
        
        Font_ColorSet_Button = QPushButton("字体颜色详细设置")
        gridLayout.addWidget(Font_ColorSet_Button, row, 1)
        Font_ColorSet_Button.clicked.connect(self.OnFort_Color)
        self.Fort_color_Lable=QLabel(" ")
        gridLayout.addWidget(self.Fort_color_Lable, row, 2)
        row = row + 1
        
        BGPicture_Set_Button = QPushButton("背景图片详细设置")
        gridLayout.addWidget(BGPicture_Set_Button, row, 1)
        BGPicture_Set_Button.clicked.connect(self.onPictureEidt)
        self.PIC_Lable=QLabel("剪裁需要的图片")
        gridLayout.addWidget(self.PIC_Lable, row, 2)
        row = row + 1            
        hbox.addLayout(gridLayout)
        action_button = QPushButton("Go 显示")
    ...
    def Group2(self, grpname):
        ...
        self.QGNAMEDP.setPlainText("安夏电子席卡 内容模拟显示区域")
        #字体横向居中
        ...
        #添加显示
        QGW.scale(QGW.width()/QGP.width(),QGW.width()/QGP.width())
        QGW.setScene(QGP)
        hbox.addWidget(QGW)
        group.setLayout(hbox)
    ...
    def initUI(self):              
        ...
        self.vlayout2.addWidget(self.Group2("模拟显示"))
    ...
    #编辑文本
    def Set_Name(self):
        self.name_input_set, ok = QInputDialog.getText(self, "请输入名字：", "输入名字：", QLineEdit.Normal, QDir.home().dirName())
...
class DisplayWindow(QDialog):
    ...
    def LayoutSetUp(self):        
        ...
        #设置文字初始化
        self.DP_QG_NAMEDP.setFont(QFont("黑体",50))#设置字体
...
class ImageViewer(QDialog):
    ...
    def Group_Picture_Show(self, grpname):
        ...
        #加载图片
        self.PE_BGPicture=QPixmap("AXLogo.png")#读取图片 AXLogo.png
        ...
        #添加显示
        PE_QGView.scale(PE_QGView.width()/PE_QGScene.width(),PE_QGView.width()/PE_QGScene.width())
