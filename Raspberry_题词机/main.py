...
class FontViewer(QDialog):
    def __init__(self,QGImage,QGImage_Zoom_R,QGImage_Rote_R,QGImage_Opacity_R,QGImage_Move_W,QGImage_Move_H,Name_Input,Name_Font,Name_Color,Name_Opacity,Name_Move_W,Name_Move_H):
        ...
        #建立共有变量：0.图片 1.放大因数 2.旋转因数 3.透明因数 4.横向移动 5.纵向移动 6.名字 7.字体 8.字体颜色 9.字体透明度 10.字体横向位置 11.字体纵向位置
        ...
        if Name_Font != None:
            ...
            self.FE_Name_Font=QFont("华文新魏",130,-1,False)
            #原始设置
    ...
    def Group_Control(self, grpname):
        ...
        self.FE_Opacity_slider.setValue(100)
        self.FE_Opacity_slider.setRange(0,100)
    ...
    def ZFModelFunction(self):
        self.FE_Name_Font=QFont('华文新魏', 350,-1,0)
        self.FE_font_R0="华文新魏"
        self.FE_font_R1=350
        self.FE_Fort_Lable.setText("%s %d" %(self.FE_font_R0,self.FE_font_R1))
        self.FE_Name_Move_W=0
        self.FE_Name_Move_H=int(-82)
        self.FE_Name_Opacity=1
        self.Set_FE_Font()
        self.OnAction()
    
    def FontResetFunction(self):
        self.FE_Name_Font=QFont('华文新魏', 150,-1,0)
        self.FE_font_R0="华文新魏"
        self.FE_font_R1=150
        self.FE_Fort_Lable.setText("%s %d" %(self.FE_font_R0,self.FE_font_R1))
        self.FE_Name_Move_W=0
    ...
    def FE_Opacity(self,FE_Opacity_Rate):
        #设置透明
        if isinstance(FE_Opacity_Rate, int) :
            self.FE_Name_Opacity=float(FE_Opacity_Rate) / 100.00
        else:
            self.FE_Name_Opacity=FE_Opacity_Rate
        self.FE_QGNAMEDP.setOpacity(float(self.FE_Name_Opacity))
        self.FE_Opacity_slider.setValue(int(self.FE_Name_Opacity*100))
        self.FE_Opacity_spinBox.setValue(self.FE_Name_Opacity)
...
class ImageViewer(QDialog):
    def __init__(self,QGImage,QGImage_Zoom_R,QGImage_Rote_R,QGImage_Opacity_R,QGImage_Move_W,QGImage_Move_H):
        ...
        self.PE_Zoom_R=float(QGImage_Zoom_R)
        self.PE_Rote_R=QGImage_Rote_R
        self.PE_Opacity_R=float(QGImage_Opacity_R)
    ...
    def Group_Control(self, grpname):
        ...
        Picture_Reset_button.clicked.connect(self.ResetPicture)
        ...
        self.PE_Zoom_spinBox.setValue(1.00)
        ...
        self.PE_Zoom_slider.setValue(100)
        self.PE_Zoom_slider.setRange(0,2000)
        ...
        self.PE_Opacity_slider.setValue(100)
        self.PE_Opacity_slider.setRange(0,100)
    ...
    def open(self):
        ...
        if fileName:
            ...
            self.PE_QGPixmap.setOpacity(1.0)#设置透明度
            ...
            self.PE_Zoom_spinBox.setValue(1.0)
            self.PE_Zoom_slider.setValue(100)
    ...
    def PE_Zoom(self,PE_ZoomScale):
        #传递 放大系数
        if  isinstance(PE_ZoomScale, int) :
            self.PE_Zoom_R=float(PE_ZoomScale) / 100.00
        else: 
            self.PE_Zoom_R=float(PE_ZoomScale)
        self.PE_QGPixmap.setScale(float(self.PE_Zoom_R))
        #设置中心点
        self.PE_QGPixmap.setOffset(-float(self.PE_BGPicture.width()/2),-float(self.PE_BGPicture.height()/2))
        #设置位置
        self.PE_QGPixmap.setPos(self.PE_Screen.width()/2+self.PE_Move_W,self.PE_Screen.height()/2+self.PE_Move_H)
        
        self.PE_Zoom_slider.setValue(int(self.PE_Zoom_R*100))
        self.PE_Zoom_spinBox.setValue(float(self.PE_Zoom_R))
    ...
    def PE_Opacity(self,PE_Opacity_Rate):
        #设置透明
        if isinstance(PE_Opacity_Rate, int) :
            self.PE_Opacity_Rate=float(PE_Opacity_Rate) / 100.00
        else:
            self.PE_Opacity_Rate=PE_Opacity_Rate
        self.PE_Opacity_R=PE_Opacity_Rate
        self.PE_QGPixmap.setOpacity(float(self.PE_Opacity_Rate))
        self.PE_Opacity_slider.setValue(int(self.PE_Opacity_Rate*100))
        self.PE_Opacity_spinBox.setValue(float(self.PE_Opacity_Rate))
    ...
    def fitToWindow(self):
        ...
        self.PE_QGPixmap.setPos(self.PE_Screen.width()/2+self.PE_Move_W,self.PE_Screen.height()/2+self.PE_Move_H)
        #重置所有参数
        self.PE_Zoom_slider.setValue(int(BGPICScale*100))
